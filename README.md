...
|----------|---------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|------------------------------|
| `GET`    | `/tasks`      | List all tasks. Add `?search=milk` for a case-insensitive title substring match (SQL `LIKE`), and/or `?done=true` / `?done=false` to filter by status (SQL `WHERE done = ?`). Filters combine, all in the database. | `200` + JSON array   | —                            |
